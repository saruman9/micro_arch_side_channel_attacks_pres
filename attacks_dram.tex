...
\begin{frame}{\insertsubsubsection}
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}[
      ...
      \draw<2, 5> (cpu) -- node[sloped, above] {Reading} (dram.two west);

      \draw<3> (dram.two east) -- ++(1, 0) |- node[right] {Copy} (buf);

      \draw<4> (buf) -- (cpu);

      \draw<5> (buf) -- node[sloped, above] {Faster!} (cpu);


    \end{tikzpicture}
    \only<1>{\caption{Reading from DRAM}}\label{fig:draw_row_buffer}

    \only<2>{\caption{CPU reads row 1, row buffer empty}}

    % \only<3>{\caption{DRAM открывает строку №1}}

    % \only<4>{\caption{CPU читает строку №1 из буфера строки}}

    \only<5>{\caption{CPU reads row 1, row buffer now full}}
  \end{figure}
