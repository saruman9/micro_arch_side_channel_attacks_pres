\begin{frame}{\insertsubsection}
  ...
  \begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Timing attack --- attack exploiting differences in the execution
      time of an algorithm}\label{cache_faster}
  \end{figure}
...
\begin{frame}{\insertsubsubsection}
  \begin{enumerate}
  \item Map binary (e.g., shared object) into address space
  \item Flush a cache line (code or data) from the cache
  \item Schedule the victim's program
  \item Check if corresponding cache line from step 2 has been loaded by the
    victim's program
  \end{enumerate}
...
\begin{frame}{\insertsubsubsection}
  ...
  \only<1>{Map binary (e.g., shared object) into address space}%

  \only<2>{Flush a cache line (code or data) from the cache}%

  \only<3>{Schedule the victim's program}%

  \only<4>{Check if corresponding cache line from step 2 has been loaded by the
    victim's program}%
