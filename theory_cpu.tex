...
\begin{frame}{\insertsubsubsection}
  ...
  \note{
    ...
    \textbf{удаляется}. Другая идея заключается в том, что процессор выполняет
    инструкции не по порядку: инструкции, которые зависят от каких-либо данных
    ожидают своего выполнения, пока инструкции, которые ни от чего не зависят
    выполняются.
